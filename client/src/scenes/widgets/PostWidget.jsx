...
const PostWidget = ({
  ...
  const patchLike = async () => {
    const response = await fetch(`http://3.136.250.119:3001/posts/${postId}/like`, {
      method: "PATCH",
      headers: {
  ...
  const handleCommentSubmit = async () => {
    try {
      const response = await fetch(`http://3.136.250.119:3001/posts/${postId}/comment`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleConfirmDelete = async () => {
    try {
      await fetch(`http://3.136.250.119:3001/posts/${postId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
